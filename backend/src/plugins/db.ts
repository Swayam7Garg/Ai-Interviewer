...
const inMemoryDb: { [key: string]: any[] } = {
  User: [
    ...
    {
      id: 'mock-user-123',
      email: 'mock-user-123@example.com',
      name: 'Test Candidate',
...
async function dbPlugin(fastify: FastifyInstance) {
  ...
  try {
    ...
    const mockUsers = [
      { id: 'mock-user-123', email: 'mock-user-123@example.com', name: 'Test Candidate' },
      { id: 'mock-user-id-1234', email: 'mock-user-id-1234@example.com', name: 'Test Candidate' }
    ];
    for (const mu of mockUsers) {
      await pool.query(
        `INSERT INTO "User" (id, email, name, "passwordHash", "roleTarget", "experienceLevel", provider, streak) 
         VALUES ($1, $2, $3, 'mock_hash', 'Backend SDE', 'mid', 'email', 1) 
         ON CONFLICT (id) DO NOTHING;`,
        [mu.id, mu.email, mu.name]
      );
    }
