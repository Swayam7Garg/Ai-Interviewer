...
export const SessionPage: React.FC = () => {
  ...
  const [faceDetectionStatus, setFaceDetectionStatus] = useState<'initializing' | 'ok' | 'no_face' | 'multiple_faces' | 'looking_away' | 'disabled'>('disabled');
  ...
  // Visibility change cheating detection (Tab Switching)
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        alert("⚠️ Proctoring Warning: Tab switching is prohibited during the interview! This incident has been logged.");
      }
    };
    document.addEventListener('visibilitychange', handleVisibilityChange);
    return () => document.removeEventListener('visibilitychange', handleVisibilityChange);
  }, [sessionStarted, isProctoringEnabled, proctoringWarnings, proctoringLogs, warningCount]);

  // Load TensorFlow.js and BlazeFace model from CDN
  useEffect(() => {
    if (!sessionStarted || !isProctoringEnabled) return;
    ...
    const loadScriptsAndModel = async () => {
      try {
        ...
        // Dynamic BlazeFace script loader
        ...
        console.log("BlazeFace proctoring model initialized.");
      } catch (err) {
  ...
  // Face Detection Proctoring loop
  useEffect(() => {
    ...
    const detectFaces = async () => {
      ...
      try {
        ...
        if (predictions.length === 0) {
          ...
          setWarningCount(c => {
            if (c % 10 === 0) {
              const logMsg = "No face detected in camera view";
              setProctoringLogs(prev => [...prev, { timestamp, event: logMsg }]);
          ...
          setWarningCount(c => {
            if (c % 10 === 0) {
              const logMsg = "Multiple faces detected in frame";
              setProctoringLogs(prev => [...prev, { timestamp, event: logMsg }]);
          ...
          const [start_x, start_y] = face.topLeft;
          const [end_x, end_y] = face.bottomRight;
          const width = end_x - start_x;
          const height = end_y - start_y;

          // Draw green face detection bounding box
          ...
          if (face.landmarks) {
            ...
            for (let i = 0; i < face.landmarks.length; i++) {
              const [lx, ly] = face.landmarks[i];
              ctx.beginPath();
              ctx.arc(lx, ly, 3, 0, 2 * Math.PI);
              ctx.fill();
            }

            // Head pose estimation (looking away check)
            ...
            if (rightEye && leftEye && nose) {
              ...
              if (eyeDistance > 0) {
                ...
                // Looking straight, noseOffset is ~0.5. If turned left or right, it goes < 0.28 or > 0.72
                if (noseOffset < 0.28 || noseOffset > 0.72) {
                  setFaceDetectionStatus('looking_away');
                  setWarningCount(c => {
                    if (c % 10 === 0) {
                      const logMsg = "Looking away from screen / head turned";
                      setProctoringLogs(prev => [...prev, { timestamp, event: logMsg }]);
                      setProctoringWarnings(prev => [...prev, `[${timestamp}] Warning: Looking away.`]);
                    }
                    return c + 1;
                  });
    ...
    detectionIntervalRef.current = setInterval(detectFaces, 330);
  ...
  }, [sessionStarted, isProctoringEnabled, modelLoaded, webcamStream, proctoringLogs, proctoringWarnings, warningCount]);
  ...
  return (
    <div className={`${isDark ? 'theme-joy-dark bg-background text-on-surface' : 'theme-joy bg-background text-on-surface'} min-h-screen transition-colors duration-300 relative overflow-x-hidden font-body`}>
              ...
              <div className="flex items-center justify-between p-4 bg-surface-container/40 border border-outline-variant rounded-2xl">
                <div className="flex flex-col gap-0.5 text-left">
                  <span className="text-xs font-bold text-on-surface flex items-center gap-1.5">
                    ...
                    Enable AI Proctoring (Anti-Cheat Mode)
                  </span>
                  <span className="text-[10px] text-on-surface-variant font-medium">Detects multiple faces, eye looking away, and tab switching</span>
                </div>
                <input
                  type="checkbox"
      ...
      {/* Floating Proctoring Camera Card */}
      {sessionStarted && isProctoringEnabled && (
        <div className="fixed bottom-24 right-6 md:bottom-6 md:right-6 w-52 bg-surface/90 backdrop-blur-md border border-outline-variant rounded-2xl shadow-premium z-50 overflow-hidden flex flex-col transition-all duration-300">
          <div className="p-2.5 bg-surface-container flex items-center justify-between border-b border-outline-variant">
            <span className="text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 text-on-surface">
              <span className={`h-2.5 w-2.5 rounded-full ${faceDetectionStatus === 'ok' ? 'bg-success animate-pulse' : 'bg-error animate-ping'}`} />
              AI Proctoring
            </span>
            <span className="text-[9px] font-bold text-on-surface-variant bg-surface px-1.5 py-0.5 rounded">
              W: {warningCount}
            </span>
          </div>
          <div className="relative aspect-video bg-black flex items-center justify-center">
            ...
            {/* Live status badge */}
            <div className="absolute bottom-2 left-2 bg-black/60 backdrop-blur-sm text-[9px] font-bold text-white px-2 py-0.5 rounded-full flex items-center gap-1">
              {faceDetectionStatus === 'initializing' ? 'Loading AI Model...' :
               faceDetectionStatus === 'ok' ? 'Verified' :
               faceDetectionStatus === 'no_face' ? 'No Face Detected' :
               faceDetectionStatus === 'multiple_faces' ? 'Multiple Faces' :
               faceDetectionStatus === 'looking_away' ? 'Looking Away' : 'Inactive'}
            </div>
          </div>
        </div>
      )}

      {/* Live Warning Toast Header banner when cheating behavior detected */}
      {sessionStarted && isProctoringEnabled && faceDetectionStatus !== 'ok' && faceDetectionStatus !== 'disabled' && faceDetectionStatus !== 'initializing' && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-error/95 text-white font-bold px-6 py-3 rounded-full shadow-lg border border-error-container flex items-center gap-2 animate-bounce">
          ...
          <span>
            {faceDetectionStatus === 'no_face' ? 'PROCTORING WARNING: No face detected in camera view!' :
             faceDetectionStatus === 'multiple_faces' ? 'PROCTORING WARNING: Multiple faces detected!' :
             faceDetectionStatus === 'looking_away' ? 'PROCTORING WARNING: Please look straight at the screen!' :
             'PROCTORING WARNING: Behavior deviation detected!'}
          </span>
        </div>
      )}
    </div>
